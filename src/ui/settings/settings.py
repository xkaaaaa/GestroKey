...
class Settings:
            ...
            if getattr(sys, 'frozen', False):
                ...
                if sys.platform == 'win32':
                    ...
                    if '.app/Contents/MacOS' in bundle_dir:
                        ...
                        app_path = os.path.dirname(bundle_dir.split('.app/Contents/MacOS')[0] + '.app')
                        if ' ' in app_path:
                            app_path = f'"{app_path}"'
                        return app_path
                    else:
    ...
    def _get_autostart_dir(self):
        ...
        if sys.platform.startswith('darwin'):
            ...
            autostart_dir = os.path.join(os.path.expanduser("~"), "Library", "LaunchAgents")
    ...
    def _create_macos_plist(self, app_path):
        ...
        # 在macOS上，一些命令需要完整路径
        if app_path.startswith("/usr/bin/python") or app_path.startswith("/usr/bin/python3"):
            # 确保python是完整路径
            pass
    ...
    def _create_linux_desktop(self, app_path):
        ...
        try:
            ...
            possible_icon_paths = [
                ...
                # 如果使用的是开发脚本
                os.path.abspath(os.path.join(os.path.dirname(sys.modules['__main__'].__file__), 
