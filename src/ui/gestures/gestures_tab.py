...
try:
    ...
    from ui.components.toast_notification import (
...
class GesturesPage(QWidget):
    ...
    request_dialog = pyqtSignal(
        str, str, str, object
    )  # message_type, title, message, callback
    ...
    def createGestureEditor(self, parent_widget):
        ...
        self.content_stack.setAnimationType(
    ...
    def _createEditTab(self):
        ...
        action_value_label.setSizePolicy(
            QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Preferred
        )

        self.action_value_input = HotkeyInput(placeholder="点击此处输入快捷键")
        self.action_value_input.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed
        )
        ...
        form_content_layout.addSpacerItem(
            QSpacerItem(
    ...
    def updateGestureCards(self, maintain_selected=True):
        ...
        try:
            ...
            for i, (name, gesture) in enumerate(gestures.items()):
                ...
                # 添加手势卡片
                card = self.addGestureCard(
                    name, gesture_id, direction, action_type, action_value
                )

                # 如果是之前选中的手势，重新选中
                if (
                    maintain_selected
                    and current_gesture_id is not None
                    and gesture_id == current_gesture_id
                ):
    ...
    def onGestureCardClicked(self, card):
        ...
        gesture_id = (
            card.property("id") if card.property("id") is not None else card.gesture_id
        )
        ...
        # 检查是否点击了当前已选中的卡片
        if (
            self.current_selected_card
            and self.current_selected_card[0] == name
            and self.current_selected_card[1] == gesture_id
        ):
            # 如果点击的是已选中的卡片，取消选中
    ...
    def addNewGesture(self):
        ...
        try:
            ...
            action_type = (
                "shortcut" if action_type_display == "执行快捷键" else action_type_display
            )
            ...
            success = self.gestures.add_gesture(
                name, direction, action_type, action_value, gesture_id=next_id
            )
    ...
    def _setup_connections(self):
        ...
        self.action_type_combo.currentIndexChanged.connect(
            self.action_type_combo_changed
        )
        self.action_value_input.hotkeyChanged.connect(
            self.action_value_input_textChanged
        )
        ...
        try:
            if is_rename:
                ...
                self.gestures.add_gesture(
                    name, direction, action_type, action_value, current_id
                )
                ...
                # 添加或更新手势 - 这会标记有未保存更改
                result = self.gestures.add_gesture(
                    name, direction, action_type, action_value, gesture_id
                )
    ...
    def deleteGesture(self):
        ...
        self.request_dialog.emit(
            "question", "确认删除", f"确定要删除手势 '{name}' 吗?", self._handle_delete_response
        )
            ...
            try:
                ...
                for name, gesture in gestures.items():
                    ...
                    if action.get("type") == "shortcut":
                        ...
                        if action_value:
                            # 转换为当前平台的快捷键格式
                            new_value = self._convert_shortcut_for_platform(
                                action_value
                            )
                            # 如果格式有变化，更新手势
                            if new_value != action_value:
                                self.logger.info(
                                    f"转换手势快捷键格式: {name}: {action_value} -> {new_value}"
                                )
    ...
    def dumpCardContents(self, name=None):
        ...
        if name and name in self.gesture_cards:
            ...
            if content_widget:
                ...
                self.logger.debug(
                    f"卡片 '{name}' 内容: 标题={card.get_title()}, {direction_text}, {action_text}"
                )
        else:
            # 打印所有卡片的内容
            for card_name, card in self.gesture_cards.items():
                ...
                if content_widget:
                    ...
                    action_text = content_widget.action_label.text()
                    self.logger.debug(
                        f"卡片 '{card_name}' 内容: 标题={card.get_title()}, {direction_text}, {action_text}"
                    )
    ...
    def updateCardContent(self, name, gesture):
        ...
        try:
            ...
            if success:
                self.logger.debug(
                    f"直接更新卡片内容成功: {name} - 方向: '{direction}', 动作: '{action_type} - {action_value}'"
                )
            else:
