...
class AnimatedStackedWidget(QStackedWidget):
    ...
    horizontal_offset = pyqtProperty(
        float, _get_horizontal_offset, _set_horizontal_offset
    )
    ...
    def setAnimationType(self, animation_type):
        ...
        if animation_type in [
            self.ANIMATION_LEFT_TO_RIGHT,
            self.ANIMATION_RIGHT_TO_LEFT,
            self.ANIMATION_TOP_TO_BOTTOM,
            self.ANIMATION_BOTTOM_TO_TOP,
            self.ANIMATION_FADE,
        ]:
...
class AnimatedNavigationButton(QWidget):
    ...
    def _setup_animations(self):
        ...
        # 指示器透明度动画
        self._indicator_opacity_animation = QPropertyAnimation(
        ...
        if self._indicator_opacity > 0:
            ...
            if self._orientation == 0:  # 垂直导航模式 - 指示器在左侧
                ...
                indicator_rect = QRectF(
                    indicator_x, self.height() - 4, indicator_width, 4
                )
                painter.fillRect(indicator_rect, indicator_color)
        ...
        if self._text:
            ...
            if self._selected:
                ...
                if self._highlight_opacity > 0:
                    text_color = QColor(
                        int(
                            self._text_color.red() * (1 - self._highlight_opacity)
                            + self._text_selected_color.red() * self._highlight_opacity
                        ),
                        int(
                            self._text_color.green() * (1 - self._highlight_opacity)
                            + self._text_selected_color.green()
                            * self._highlight_opacity
                        ),
                        int(
                            self._text_color.blue() * (1 - self._highlight_opacity)
    ...
    # 定义属性
    highlight_opacity = pyqtProperty(
        float, _get_highlight_opacity, _set_highlight_opacity
    )
    indicator_position = pyqtProperty(
        float, _get_indicator_position, _set_indicator_position
    )
    indicator_opacity = pyqtProperty(
...
class SideNavigationMenu(QWidget):
    ...
    def _createGroupContainers(self):
        ...
        self._group_containers = {
            ...
            "default_top": {
                ...
                "widget": self._top_container,
                "layout": self._top_layout,
            },
            "default_bottom": {
                "position": self.POSITION_BOTTOM,
                "widget": self._bottom_container,
    ...
    def createGroup(self, group_name, position=POSITION_TOP, title=None):
        ...
        if title:
            ...
            if self._orientation == self.ORIENTATION_VERTICAL:
                title_label.setStyleSheet(
                    "font-size: 11px; color: #666; margin-top: 5px; margin-bottom: 2px; padding: 2px 5px;"
                )
            else:
                title_label.setStyleSheet(
                    "font-size: 11px; color: #666; margin-right: 5px; padding: 2px 5px;"
                )
        ...
        parent_layout = (
            self._top_layout if position == self.POSITION_TOP else self._bottom_layout
        )
    ...
    def addPage(self, widget, text, icon=None, position=POSITION_TOP, group_name=None):
        ...
        """
        # 创建导航按钮
        button = AnimatedNavigationButton(
            text, icon, parent=self, orientation=self._orientation
        )
        ...
        if group_name and group_name in self._group_containers:
            ...
            # 设置默认分组名称
            group_name = (
                "default_top" if position == self.POSITION_TOP else "default_bottom"
            )
        ...
        self.logger.debug(
            f"添加页面: {text}, 位置: {position}, 分组: {group_name}, 索引: {len(self._buttons) - 1}"
        )
    ...
    def setCurrentPage(self, index):
        ...
        if self._animations_enabled and self._current_index != -1:
            ...
            current_index_in_container = current_container_layout.indexOf(
                current_button
            )
            ...
            if self._orientation == self.ORIENTATION_VERTICAL:
                ...
                if (
                    ...
                    and current_group == target_group
                ):
                    # 在同一个位置区域和同一分组内的切换
                    if current_index_in_container < target_index_in_container:
                        # 从上到下切换时，内容从下方进入
                        self._stack.setAnimationType(
                            AnimatedStackedWidget.ANIMATION_BOTTOM_TO_TOP
                        )
                    else:
                        # 从下到上切换时，内容从上方进入
                        self._stack.setAnimationType(
                            AnimatedStackedWidget.ANIMATION_TOP_TO_BOTTOM
                        )
                else:
                    # 跨位置区域或分组的切换
                    if (
                        current_position == self.POSITION_TOP
                        and target_position == self.POSITION_BOTTOM
                    ):
                ...
                if (
                    ...
                    if current_index_in_container < target_index_in_container:
                        # 从左到右切换时，内容从右侧进入
                        self._stack.setAnimationType(
                            AnimatedStackedWidget.ANIMATION_RIGHT_TO_LEFT
                        )
                    else:
                        # 从右到左切换时，内容从左侧进入
                        self._stack.setAnimationType(
                            AnimatedStackedWidget.ANIMATION_LEFT_TO_RIGHT
                        )
                else:
                    # 跨位置区域或分组的切换
                    if (
    ...
    def setPagePosition(self, index, position, group_name=None):
        ...
        self.logger.debug(
            f"页面 {index} 位置变更: {old_position} -> {position}, 分组: {old_group} -> {group_name}"
        )
...
if __name__ == "__main__":
    ...
    nav_menu_vertical = SideNavigationMenu(
    ...
    # 创建分组
    nav_menu_vertical.createGroup(
        "main_features", nav_menu_vertical.POSITION_TOP, "主要功能"
    )
    ...
    nav_menu_vertical.addPage(
        page5, "帮助", QIcon(), nav_menu_vertical.POSITION_BOTTOM, "help"
    )
    nav_menu_vertical.addPage(
        page6, "关于", QIcon(), nav_menu_vertical.POSITION_BOTTOM, "help"
    )
    ...
    nav_menu_horizontal = SideNavigationMenu(
        orientation=SideNavigationMenu.ORIENTATION_HORIZONTAL
    )
    ...
    # 创建分组 - 水平模式
    nav_menu_horizontal.createGroup(
        "main_features", nav_menu_horizontal.POSITION_TOP, "主要功能"
    )
    nav_menu_horizontal.createGroup(
        "settings", nav_menu_horizontal.POSITION_TOP, "设置选项"
    )
    nav_menu_horizontal.createGroup(
        "help", nav_menu_horizontal.POSITION_BOTTOM, "帮助与支持"
    )

    # 添加页面 - 水平模式
    nav_menu_horizontal.addPage(
        h_page1, "控制台", console_icon, group_name="main_features"
    )
    nav_menu_horizontal.addPage(
        h_page3, "手势管理", gestures_icon, group_name="main_features"
    )
    ...
    nav_menu_horizontal.addPage(
        h_page5, "帮助", QIcon(), nav_menu_horizontal.POSITION_BOTTOM, "help"
    )
    nav_menu_horizontal.addPage(
        h_page6, "关于", QIcon(), nav_menu_horizontal.POSITION_BOTTOM, "help"
    )
