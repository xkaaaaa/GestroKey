...
class ColorSwatch(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.selected:
            ...
            painter.drawEllipse(
                QPointF(center_x, center_y), actual_radius + 1, actual_radius + 1
            )
    ...
    def showEvent(self, event):
        ...
        try:
            ...
            if hasattr(self, "scaleAnimation") and self.scaleAnimation:
                # 设置起始尺寸为80%
                start_size = QSize(
                    int(target_size.width() * 0.8), int(target_size.height() * 0.8)
                )
...
class AnimatedColorPicker(QWidget):
    ...
    def open_color_dialog(self):
        ...
        if main_window:
            ...
            dialog_y = (
                parent_rect.y()
                + (parent_rect.height() - self.color_dialog.height()) // 2
            )
    ...
    def _get_main_window(self):
        ...
        while parent:
            # 检查窗口是否是顶级窗口或主窗口
            if isinstance(parent, QWidget) and (
                parent.isWindow() or parent.objectName() == f"{APP_NAME}App"
            ):
                return parent
...
if __name__ == "__main__":
    ...
    color_display.setStyleSheet(
        "background-color: rgb(231, 76, 60); border-radius: 5px;"
    )
