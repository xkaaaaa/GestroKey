...
class SpinnerButton(QWidget):
    ...
    def paintEvent(self, event):
        ...
        painter.drawEllipse(
            QRectF(
                center_x - scale_size / 2,
                center_y - scale_size / 2,
                scale_size,
                scale_size,
            )
        )
...
class AnimatedNumberSpinner(QWidget):
    ...
    def __init__(
        ...
        self.logger.debug(
            f"数字选择器初始化, 值: {self._value}, 范围: {min_value}-{max_value}, 步长: {step}"
        )
    ...
    def initUI(self):
        ...
        self.subtract_button = SpinnerButton(
            button_type="subtract", size=28, primary_color=self._primary_color
        )
        ...
        self.add_button = SpinnerButton(
            button_type="add", size=28, primary_color=self._primary_color
        )
