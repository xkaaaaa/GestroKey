...
class CustomComboBox(QComboBox):
    ...
    def hidePopup(self):
        ...
        if popup and popup.isVisible():
            ...
            height_anim.finished.connect(
                lambda: super(CustomComboBox, self).hidePopup()
            )
    ...
    def _updateStyleSheet(self):
        ...
        """ % (
            self._backgroundColor.name(),
            self._borderColor.name(),
    ...
    def paintEvent(self, event):
        ...
        # 计算当前颜色（基于动画进度）
        bgColor = self._interpolateColor(
            self._backgroundColor,
            self._interpolateColor(
                self._backgroundHoverColor,
        ...
        borderColor = self._interpolateColor(
            self._borderColor,
            self._interpolateColor(
                self._hoverBorderColor, self._pressBorderColor, self._pressProgress
            ),
            self._hoverProgress,
        )

        textColor = self._interpolateColor(
            self._textColor, self._textHoverColor, self._hoverProgress
        )

        arrowColor = self._interpolateColor(
            self._arrowColor,
            self._interpolateColor(
                self._arrowHoverColor, self._arrowPressColor, self._pressProgress
            ),
            self._hoverProgress,
        )
    ...
    def setAnimationEasingCurve(
        self,
        hover_curve=QEasingCurve.Type.OutCubic,
    ...
    def customizeCustomComboBox(self, **customValues):
        ...
        if "arrowAnimationDuration" in customValues:
            self._arrowRotationAnimation.setDuration(
                customValues["arrowAnimationDuration"]
            )
...
class ComboBoxDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        elidedText = metrics.elidedText(
            text, Qt.TextElideMode.ElideRight, textRect.width()
        )
