import sys
import os
from PyQt6.QtWidgets import QCheckBox, QWidget, QApplication, QHBoxLayout, QVBoxLayout
from PyQt6.QtCore import Qt, QPropertyAnimation, QEasingCurve, QRect, QSize, pyqtProperty, pyqtSignal, QPoint
...
class AnimatedCheckBox(QCheckBox):
    ...
    def _handle_state_changed(self, state):
        ...
        try:
            ...
            self.logger.debug(f"复选框状态变为: {'选中' if state == Qt.CheckState.Checked.value else '未选中'}")
        except Exception as e:
    ...
    def paintEvent(self, event):
        ...
        try:
            ...
            # 计算位置
            box_rect = QRect(self._padding, (self.height() - self._box_size) // 2, 
            ...
            if self._animation_progress > 0.0:
                ...
                p1 = QPoint(int(box_rect.left() + margin), 
                            int(box_rect.top() + self._box_size * 0.5))
                p2 = QPoint(int(box_rect.left() + self._box_size * 0.4), 
                            int(box_rect.bottom() - margin))
                p3 = QPoint(int(box_rect.right() - margin), 
                            int(box_rect.top() + margin))
    ...
    animationProgress = pyqtProperty(float, _get_animation_progress, _set_animation_progress)
