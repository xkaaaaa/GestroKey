...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QPushButton, QWidget, QVBoxLayout, QHBoxLayout,
                             QLabel, QGraphicsDropShadowEffect, QGraphicsBlurEffect, QComboBox, QSizePolicy,
                             QSpacerItem, QGraphicsOpacityEffect, QLineEdit, QCheckBox, QTabWidget, QScrollArea,
                             QFrame, QMessageBox, QInputDialog, QFileDialog, QColorDialog, QFontDialog)
from PyQt6.QtCore import QEasingCurve, QPropertyAnimation, QRect, QPoint, Qt, pyqtProperty, QSize, QTimer, QEvent
from PyQt6.QtGui import QColor, QFont, QLinearGradient, QBrush, QPalette, QPainter, QIcon, QPixmap
...
class MessageDialog(QWidget): # 对话框组件
    ...
    """
    def __init__(self, message_type="warning", content_widget=None, parent=None, show_title=True, show_buttons=True, 
                 title_text="消息提示", message="", custom_icon=None, custom_buttons=None, custom_button_colors=None,
        ...
        if show_title:
            ...
            # 添加图标
            icon_label = QLabel(MESSAGE_TYPES[message_type]["icon"] if self.custom_icon is None else self.custom_icon)
            icon_label.setStyleSheet(f"font-size: {CONFIG['font_size_base']+8}pt; color: {MESSAGE_TYPES[message_type]['color']};")
            header_layout.addWidget(icon_label)
            
            # 添加标题
            title = QLabel(title_text)
            title.setStyleSheet(f"font-size: {CONFIG['font_size_base']+5}pt; font-weight: 600; color: {CONFIG['color_text_dark']}; background-color: transparent;")
            ...
            close_btn.setStyleSheet(f"""QPushButton {{ background: none; border: none; font-size: {CONFIG['font_size_base']+5}pt; color: {CONFIG['color_text_dark']}; padding: 0 5px; background-color: transparent; }}
        ...
        if message:
            ...
            message_label.setStyleSheet(f"color: {CONFIG['color_text_dark']}; line-height: 1.6;")
        ...
        if show_buttons:
            ...
            for button_text in buttons_to_show:
                # 获取按钮颜色
                button_color = MESSAGE_TYPES[message_type]["button_colors"].get(button_text, CONFIG["color_btn_secondary_bg"])
                if self.message_type == "custom" and self.custom_button_colors and button_text in self.custom_button_colors:
                    button_color = self.custom_button_colors[button_text]
                
                btn = AnimatedButton(button_text, primary_color=button_color)
                btn.clicked.connect(lambda checked, text=button_text: self.handle_button_click(text))
    ...
    def _get_target_widget(self):
        ...
        # 如果父窗口是主窗口，直接使用主窗口
        if hasattr(parent, 'centralWidget') and callable(getattr(parent, 'centralWidget')):
    ...
    def remove_parent_blur(self):
        ...
        try:
            # 停止正在进行的动画
            if hasattr(self, 'fade_anim') and self.fade_anim and self.fade_anim.state() == QPropertyAnimation.State.Running:
                self.fade_anim.stop()
    ...
    def getOverlayOpacity(self):
        if hasattr(self, 'overlay_widget') and self.overlay_widget:
            ...
            if "rgba(0, 0, 0, " in style:
                try:
                    opacity = int(float(style.split("rgba(0, 0, 0, ")[1].split(")")[0]) * 255)
                    return opacity
                except:
                    return 0
        return 0
    ...
    def setOverlayOpacity(self, opacity):
        if hasattr(self, 'overlay_widget') and self.overlay_widget:
            ...
            self.overlay_widget.setStyleSheet(f"background-color: rgba(0, 0, 0, {opacity_float});")
    ...
    def paintEvent(self, event):
        ...
        try:
            ...
            painter.drawRoundedRect(self.rect(), CONFIG['radius_modal'], CONFIG['radius_modal'])
    ...
    def show_animated(self): # 带动画显示
        ...
        self.opacity_anim.setStartValue(0.0); self.opacity_anim.setEndValue(1.0); self.opacity_anim.start()
        self.geom_anim.setStartValue(start_geom); self.geom_anim.setEndValue(end_geom); self.geom_anim.start()
    ...
    def close_animated(self): # 带动画关闭
        ...
        end_geom.setSize(QSize(int(current_geom.width() * CONFIG["scale_anim_factor"]), int(current_geom.height() * CONFIG["scale_anim_factor"])))
        end_geom.moveCenter(current_geom.center())
        self.opacity_anim.setStartValue(1.0); self.opacity_anim.setEndValue(0.0)
        self.geom_anim.setStartValue(current_geom); self.geom_anim.setEndValue(end_geom)
        # 两个动画都结束后隐藏
        finished_count = 0
        def check_finish():
        ...
        self.opacity_anim.start(); self.geom_anim.start()
...
# 公开函数，用于显示对话框
def show_dialog(parent, message_type="warning", title_text=None, message="", content_widget=None, 
                custom_icon=None, custom_buttons=None, custom_button_colors=None, callback=None):
    """
    显示对话框
...
if __name__ == '__main__':
    ...
    class TestWindow(QMainWindow):
        def __init__(self):
            ...
            p = self.palette(); grad = QLinearGradient(0, 0, 0, self.height())
            grad.setColorAt(0.0, QColor(CONFIG["color_primary"])); grad.setColorAt(1.0, QColor(CONFIG["color_secondary"]))
            p.setBrush(QPalette.ColorRole.Window, QBrush(grad)); self.setPalette(p)
            ...
            title_label.setStyleSheet(f"font-size: {CONFIG['font_size_base']+15}pt; font-weight: 600; color: {CONFIG['color_text_light']}; margin-bottom: 10px;")
            title_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
            
            desc_label = QLabel("测试不同类型的选择对话框")
            desc_label.setStyleSheet(f"font-size: {CONFIG['font_size_base']+2}pt; color: {CONFIG['color_text_light']}; opacity: 0.9; margin-bottom: 15px; line-height: 1.6;")
            ...
            for msg_type, info in MESSAGE_TYPES.items():
                if msg_type != "custom":  # 跳过custom类型，因为移除了自定义设置功能
                    ...
                    btn.setStyleSheet(f"""QPushButton {{ background-color: {info['color']}; color: {CONFIG['color_white']}; border: none;
                                        padding: 10px 20px; border-radius: {CONFIG['radius_button']}px; font-size: {CONFIG['font_size_base']}pt; }}
                                    QPushButton:hover {{ opacity: 0.9; }}""")
                    btn.setCursor(Qt.CursorShape.PointingHandCursor)
                    btn.clicked.connect(lambda checked, t=msg_type: self.show_test_dialog(t))
    ...
    try:
        ...
        if main_window and hasattr(main_window, "show_global_dialog"):
            ...
            page.request_dialog.connect(
                lambda message_type, title, message, callback:
                main_window.show_global_dialog(
                    message_type=message_type,
                    title_text=title,
                    message=message,
                    callback=callback
            ...
            page.logger.warning(f"未找到主窗口或主窗口没有show_global_dialog方法，找到的窗口类型: {type(main_window)}")
            return False
    except Exception as e:
