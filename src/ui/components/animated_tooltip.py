...
class AnimatedToolTip(QWidget):
    ...
    def __init__(
        ...
        # 样式属性 - 默认使用主题蓝色
        self._primary_color = (
            self._parse_color(primary_color) if primary_color else QColor(52, 152, 219)
        )  # 背景色 - 天蓝色
        self._text_color = (
            self._parse_color(text_color) if text_color else QColor(255, 255, 255)
        )  # 文本色 - 白色
    ...
    def initUI(self):
        ...
        self.label.setStyleSheet(
            f"color: rgba({self._text_color.red()}, {self._text_color.green()}, {self._text_color.blue()}, 255);"
        )
    ...
    def setPrimaryColor(self, color):
        ...
        self.update()  # 触发重绘
        self.logger.debug(
            f"设置主题颜色: RGB({self._primary_color.red()}, {self._primary_color.green()}, {self._primary_color.blue()})"
        )
    ...
    def setTextColor(self, color):
        ...
        self.logger.debug(
            f"设置文本颜色: RGB({self._text_color.red()}, {self._text_color.green()}, {self._text_color.blue()})"
        )
    ...
    def updateSize(self):
        ...
        # 计算文本宽度（限制在最大宽度以内）
        text_width = min(
            metrics.horizontalAdvance(self.text), self.max_width - 2 * self.padding
        )
        ...
        class GlobalEventFilter(QObject):
            ...
            def eventFilter(self, obj, event):
                if event.type() == QEvent.Type.MouseMove:
                    ...
                    app = QApplication.instance()
                    app.setProperty(
                        "_last_mouse_move_time", QDateTime.currentMSecsSinceEpoch()
                    )
                    ...
                    if (
                        self.last_pos.isNull()
                        or (pos - self.last_pos).manhattanLength() > 3
                    ):
                        ...
                        if widget in AnimatedToolTip._registered_tooltips:
                            ...
                            if widget != self.current_widget:
                                ...
                                if (
                                    ...
                                    in AnimatedToolTip._registered_tooltips
                                ):
                                    tooltip, _ = AnimatedToolTip._registered_tooltips[
                                        self.current_widget
                                    ]
                                    ...
                                    if AnimatedToolTip._current_visible_tooltip:
                                        AnimatedToolTip._current_visible_tooltip.hideTooltip(
                                            force=True
                                        )
                                    ...
                                    self.leave_timer.timeout.connect(
                                        lambda: tooltip.hideTooltip()
                                    )
                                ...
                                self.hover_timer.timeout.connect(
                                    lambda: tooltip.showTooltip()
                                )
                                self.hover_timer.start(tooltip.show_delay)

                                self.logger.debug(
                                    f"鼠标进入组件: {widget.__class__.__name__}"
                                )
                        else:
                            ...
                            if (
                                ...
                                self.logger.debug(
                                    f"鼠标离开组件: {self.current_widget.__class__.__name__}"
                                )
    ...
    def showTooltip(self):
        ...
        # 单例模式：先隐藏所有其他提示
        if (
            AnimatedToolTip._current_visible_tooltip
            and AnimatedToolTip._current_visible_tooltip != self
        ):
        ...
        # 根据弹出方向确定动画起始位置
        if (
            self.direction == self.DIRECTION_BOTTOM
            or self.direction == self.DIRECTION_AUTO
        ):
            # 从下方弹出（或自动选择默认方向）
            start_rect = QRect(
                start_pos.x(), start_pos.y() + 10, self.width(), self.height()
            )
            ...
            # 从左侧弹出
            start_rect = QRect(
                start_pos.x() - 10, start_pos.y(), self.width(), self.height()
            )
    ...
    def check_mouse_position(self):
        ...
        target_rect = QRect(
            global_pos.x(), global_pos.y(), target_geo.width(), target_geo.height()
        )
    ...
    def hideTooltip(self, force=False):
        ...
        start_rect = QRect(
            current_pos.x(), current_pos.y(), self.width(), self.height()
        )
        ...
        if (
            ...
            # 向下淡出
            end_rect = QRect(
                current_pos.x(), current_pos.y() + 10, self.width(), self.height()
            )
        elif self.direction == self.DIRECTION_TOP:
            # 向上淡出
            end_rect = QRect(
            ...
            # 向左淡出
            end_rect = QRect(
                current_pos.x() - 10, current_pos.y(), self.width(), self.height()
            )
        elif self.direction == self.DIRECTION_RIGHT:
            # 向右淡出
            end_rect = QRect(
    ...
    def eventFilter(self, obj, event):
        ...
        if event.type() == QEvent.Type.Enter or event.type() == QEvent.Type.HoverEnter:
            ...
            event.type() == QEvent.Type.Leave or event.type() == QEvent.Type.HoverLeave
        ):
            # 鼠标离开目标控件
            if self.fade_timer:
        ...
        class SpecialFilter(QObject):
            ...
            def eventFilter(self, obj, event):
                if event.type() == QEvent.Type.Enter:
                    ...
                    if (
                        tooltip_instance
                        and hasattr(tooltip_instance, "target_widget")
                        and tooltip_instance.target_widget == self.widget
                    ):
                        QTimer.singleShot(
                            tooltip_instance.show_delay, tooltip_instance.showTooltip
                        )
                    ...
                    if (
                        ...
                        QTimer.singleShot(
                            tooltip_instance.hide_delay, tooltip_instance.hideTooltip
                        )
                return False
...
def wrap_widget_with_tooltip(
    widget,
    text,
    tooltip=None,
    show_delay=1000,
    direction=AnimatedToolTip.DIRECTION_AUTO,
    primary_color=None,
    text_color=None,
    border_radius=6,
):
...
def set_tooltip(
    ...
    if not _global_tooltip:
        _global_tooltip = AnimatedToolTip(
            show_delay=show_delay,
            direction=direction,
            primary_color=primary_color,
            text_color=text_color,
...
if __name__ == "__main__":
    ...
    button = add_test_component(
        QPushButton("测试按钮"), "按钮", "QPushButton - 标准点击按钮，最常用的交互组件"
    )

    label = add_test_component(QLabel("这是一个标签"), "标签", "QLabel - 用于显示文本的静态组件")
    ...
    checkbox = add_test_component(
        create_tooltip_for_special_widget(
            checkbox_widget, "勾选后将启用某功能，未勾选则禁用", primary_color=[231, 76, 60]
        ),
        "复选框",
    ...
    text_input = add_test_component(QLineEdit(), "文本框", "QLineEdit - 单行文本输入组件")
    text_input.setPlaceholderText("在此输入文本...")

    from PyQt6.QtWidgets import QRadioButton

    radio1 = add_test_component(
        QRadioButton("选项1"), "单选按钮", "QRadioButton - 在一组选项中只能选择一个"
    )
    ...
    combobox = add_test_component(
        create_tooltip_for_special_widget(
            combobox_widget, "从下拉列表中选择一个选项", primary_color=[155, 89, 182]
        ),
        "下拉菜单",
    ...
    slider = add_test_component(
        QSlider(Qt.Orientation.Horizontal), "滑块", "QSlider - 使用滑动方式调整数值的组件"
    )
    slider.setMinimum(0)
    ...
    spinbox = add_test_component(
        create_tooltip_for_special_widget(
            spinbox_widget, "通过上下箭头或直接输入调整数值", primary_color=[52, 73, 94]
        ),
        "数字调节器",
    ...
    progress = add_test_component(
        create_tooltip_for_special_widget(
            progress_widget, "这是一个进度条，显示操作完成的百分比", primary_color=[41, 128, 185]
        ),
        "进度条",
    ...
    date_edit = add_test_component(QDateEdit(), "日期编辑器", "QDateEdit - 用于选择日期的组件")
    ...
    lcd = add_test_component(
        create_tooltip_for_special_widget(
            lcd_widget, "LCD数字显示器，目前显示数值12345", primary_color=[231, 76, 60]
        ),
        "LCD显示器",
    ...
    text_edit = add_test_component(QTextEdit(), "文本编辑器", "QTextEdit - 多行文本编辑组件")
    ...
    for row_idx in range(3):
        for col_idx in range(3):
            table.setItem(
                row_idx, col_idx, QTableWidgetItem(f"单元格 {row_idx+1},{col_idx+1}")
            )
    table.setMaximumHeight(120)
    add_test_component(table, "表格", "QTableWidget - 显示行列数据的表格组件")

    # 将网格布局添加到主布局
    layout.addLayout(grid)

    # === 为标准组件设置提示 ===
    wrap_widget_with_tooltip(
        button, "这是一个按钮的提示，用于用户点击执行操作", primary_color=[52, 152, 219]
    )
    wrap_widget_with_tooltip(
        label,
        "这是一个标签组件，用于展示静态文本",
        primary_color=[46, 204, 113],
        direction=AnimatedToolTip.DIRECTION_BOTTOM,
    )
    wrap_widget_with_tooltip(
        text_input, "请在此输入您要搜索的内容", direction=AnimatedToolTip.DIRECTION_TOP
    )
    wrap_widget_with_tooltip(radio1, "第一个选项的详细说明", show_delay=200)
    wrap_widget_with_tooltip(
        slider,
        "通过滑动调整数值，范围为0-100",
        direction=AnimatedToolTip.DIRECTION_TOP,
        primary_color=[243, 156, 18],
    )
    ...
    checkbox_wrapper = create_tooltip_for_special_widget(
        checkbox_example, "这是使用包装器的复选框提示", primary_color=[231, 76, 60]
    )
    ...
    combo_wrapper = create_tooltip_for_special_widget(
        combo_example, "这是使用包装器的下拉菜单提示", primary_color=[155, 89, 182]
    )
