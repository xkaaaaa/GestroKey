...
class AnimatedScrollBar(QScrollBar):
    ...
    def __init__(self, orientation=Qt.Orientation.Vertical, parent=None):
        ...
        # 记录初始化完成
        self.logger.debug(
            f"{'垂直' if orientation == Qt.Orientation.Vertical else '水平'}滚动条初始化完成（折叠状态）"
        )
    ...
    def _setupAnimations(self):
        ...
        self._width_animation.setDuration(
            int(self._animation_duration * 1.5)
        )  # 将浮点数转换为整数
...
class AnimatedScrollArea(QScrollArea):
    ...
    def __init__(self, parent=None):
        ...
        # 平滑滚动相关
        self._smooth_scroll_animation = QPropertyAnimation(
