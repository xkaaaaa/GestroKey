...
class AnimatedButton(QPushButton):
    """
    一个带有悬停、点击动画效果的按钮。采用扁平化设计，支持文字动画效果。
    可以自定义颜色、图标、文本和大小。
    """
    ...
    def __init__(
        ...
        # 默认参数设置 - 蓝色主题
        self._primary_color = (
            self._parse_color(primary_color) if primary_color else QColor(41, 128, 185)
        )  # 扁平化蓝色
        ...
        self._border_color = (
            self._parse_color(border_color)
            if border_color
            else self._primary_color.darker(110)
        )
        self._shadow_color = (
            self._parse_color(shadow_color) if shadow_color else QColor(0, 0, 0, 60)
        )
    ...
    def paintEvent(self, event):
        ...
        shadow_path.addRoundedRect(
            QRectF(shadow_rect), self._border_radius, self._border_radius
        )
        ...
        if not self._pressed or not self.isEnabled():
            ...
            painter.drawRoundedRect(
                button_rect, self._border_radius, self._border_radius
            )
        ...
        if not self.icon().isNull():
            ...
            if not self.isEnabled():
                ...
                self.icon().paint(
                    painter, icon_rect, Qt.AlignmentFlag.AlignCenter, icon_mode
                )
        ...
        text_rect = self.rect().adjusted(
            self.iconSize().width() + 8 if has_icon else 8, 0, -4, 0
        )
        ...
        if self.isEnabled():
            ...
            text_rect = text_rect.adjusted(
                0, int(self._text_y_offset), 0, int(self._text_y_offset)
            )
...
if __name__ == "__main__":
    ...
    buttons = [
        ...
        # 天蓝色按钮
        {
            ...
            "hover_color": [85, 175, 237],
        },
        # 浅蓝色按钮，带深色文本
        {
            "text": "浅蓝色按钮",
            "primary_color": [133, 193, 233],
            "hover_color": [169, 204, 227],
            "text_color": [33, 37, 41],
        },
        # 带图标的按钮
    ...
    for i, button_config in enumerate(buttons):
        ...
        if "icon" not in button_config and i % 2 == 0:  # 偶数索引的按钮添加图标
            ...
            button.setIcon(
                button.style().standardIcon(QStyle.StandardPixmap.SP_DialogOkButton)
            )
    ...
    for i, button in enumerate(disabled_buttons):
        if i == 1:  # 为中间的按钮添加图标
            button.setIcon(
                button.style().standardIcon(QStyle.StandardPixmap.SP_DialogSaveButton)
            )
            button.setIconSize(QSize(20, 20))
