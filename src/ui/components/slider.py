...
class GesturePattern(QWidget):
    ...
    def paintEvent(self, event):
        ...
        gradient.setColorAt(
            0.7,
            QColor(
                int(self.primary_color[0]),
                int(self.primary_color[1]),
                int(self.primary_color[2]),
                100,
            ),
        )
        gradient.setColorAt(
            1,
            QColor(
                int(self.secondary_color[0]),
                int(self.secondary_color[1]),
                int(self.secondary_color[2]),
                50,
            ),
        )
        ...
        if self.show_value:
            ...
            painter.drawText(
                QRectF(0, 0, width, height), Qt.AlignmentFlag.AlignCenter, value_text
            )
        ...
        painter.drawEllipse(
            QRectF(
                center_x - self.orbit_radius,
                center_y - self.orbit_radius,
                self.orbit_radius * 2,
                self.orbit_radius * 2,
            )
        )
        ...
        painter.drawEllipse(
            QRectF(
                center_x - self.dot_radius * 1.5,
                center_y - self.dot_radius * 1.5,
                self.dot_radius * 3,
                self.dot_radius * 3,
            )
        )
    ...
    def paintEvent(self, event):
        ...
        if self._orientation == Qt.Orientation.Horizontal:
            ...
            progress_rect = QRectF(
                0, (height - track_height) / 2, progress_width, track_height
            )
        else:
        ...
        hover_opacity = 0.7
        current_opacity = (
            base_opacity + (hover_opacity - base_opacity) * self._hover_factor
        )
        ...
        if self._hover_factor > 0:
            ...
            # 为进度条添加更强的发光效果
            progress_glow_color = QColor(
                *self._track_color, int(80 * self._hover_factor)
            )
            for i in range(2):
                blur_radius = 3 + i * 2
                ...
                painter.drawRoundedRect(
                    progress_glow_rect, 3 + blur_radius, 3 + blur_radius
                )
...
class AnimatedSlider(QWidget):
    ...
    def _animate_handle_hover(self, hovered):
        ...
        if hovered:
            ...
            self._handle_size_animation.setStartValue(self._handle.width())
            target_size = min(
                self._handle_size * 1.1,
                self._handle_size * 1.2 if self._pressed else self._handle_size * 1.1,
            )
    ...
    def _pos_to_value(self, pos):
        """将位置转换为值"""
        if self._orientation == Qt.Orientation.Horizontal:
            return self._minimum + (self._maximum - self._minimum) * (
                pos / float(self.width())
            )
        else:
            return self._minimum + (self._maximum - self._minimum) * (
                1.0 - (pos / float(self.height()))
            )
