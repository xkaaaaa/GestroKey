...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QPushButton, QVBoxLayout, 
                           QWidget, QLabel, QGridLayout, QFrame, QProgressBar, QHBoxLayout,
                           QGraphicsDropShadowEffect, QSpacerItem, QSizePolicy, QMessageBox)
from PyQt6.QtCore import (Qt, QTimer, pyqtSignal, pyqtProperty, QRectF, QPoint, 
                         QPropertyAnimation, QEasingCurve, QSequentialAnimationGroup,
                         QParallelAnimationGroup, QObject, QSize, QRect, QThread, QEvent,
                         QAbstractAnimation, QRunnable, QThreadPool, pyqtSlot, QMetaObject)
from PyQt6.QtGui import (QColor, QPainter, QPen, QBrush, QPainterPath, QLinearGradient,
                        QFont, QFontMetrics, QPixmap, QTransform, QIcon, QTextOption)
...
class ToastPreloadWorker(QRunnable):
    ...
    def _create_preloaded_toast_part1(self):
        ...
        try:
            ...
            if not hasattr(_preloaded_toast, 'opacity_animation') or not _preloaded_toast.opacity_animation:
        ...
        try:
            ...
            if size.width() > 0 and size.height() > 0:
                ...
                for i, toast_type in enumerate(toast_types):
                    ...
                    self.signals.progress_updated.emit(f"渲染{['信息', '成功', '警告', '错误'][i]}类型通知...", progress)
...
class ElegantToast(QWidget):
    ...
    def init_ui(self):
        ...
        icon_spacer = QSpacerItem(40, 30, QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        ...
        if self.text_mode == self.TEXT_TRUNCATE:
            ...
            elided_text = metrics.elidedText(self.message, Qt.TextElideMode.ElideRight, 200)
        ...
        # 使用内容对齐方式添加到布局中
        content_layout.addWidget(self.label, 1, Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignTop)
    ...
    def create_animations(self):
        ...
        if self.text_mode == self.TEXT_SCROLL:
            ...
            # 第二段动画：快速回到起始位置准备下一轮
            self.scroll_reset_animation = QPropertyAnimation(self, b"text_scroll_offset")
    ...
    def showEvent(self, event):
        ...
        # 如果是滚动模式且文本过长，开始滚动动画
        if self.text_mode == self.TEXT_SCROLL and hasattr(self, 'scroll_animation_group'):
            # 计算文本宽度
            metrics = QFontMetrics(QFont("Arial", 11))
        ...
        if self.duration > 0:
            ...
            # 移除旧的定时器逻辑，改用动画事件
            self.progress_animation.valueChanged.connect(self._on_progress_animation_value_changed)
    ...
    def _adjust_height_for_text(self):
        ...
        # 获取文本所需矩形区域
        text_rect = metrics.boundingRect(0, 0, available_width, 2000, Qt.TextFlag.TextWordWrap, self.message)
        ...
        # 记录日志
        self.logger.debug(f"简单高度计算: 行数={estimated_lines}, 行高={line_height}, " +
                         f"系数={height_factor}, 固定高度={fixed_height}, 窗口高度={window_height}")
    
    def show_notification(self, message=None, toast_type=None, duration=None, text_mode=None):
        """显示通知并重置进度条"""
        # 重置关闭状态
        self._closing = False
        ...
        if message is not None:
            ...
            if self.text_mode == self.TEXT_TRUNCATE:
                metrics = QFontMetrics(QFont("Arial", 11))
                elided_text = metrics.elidedText(message, Qt.TextElideMode.ElideRight, 200)
                self.label.setText(elided_text)
            else:
    ...
    def draw_type_icon(self, painter):
        ...
        if self.toast_type == self.INFO:
            ...
            painter.drawLine(int(center.x() - 7), int(center.y()), int(center.x() - 2), int(center.y() + 5))
            painter.drawLine(int(center.x() - 2), int(center.y() + 5), int(center.x() + 7), int(center.y() - 5))
            ...
            center = icon_rect.center()
            painter.setPen(QPen(self.icon_color, 2.5))
            painter.drawLine(int(center.x() - 6), int(center.y() - 6), int(center.x() + 6), int(center.y() + 6))
            painter.drawLine(int(center.x() - 6), int(center.y() + 6), int(center.x() + 6), int(center.y() - 6))
...
def show_info(parent, message, duration=5000, icon=None, position='top-right', text_mode=ElegantToast.TEXT_WRAP):
    """显示信息提示"""
    return show_toast(parent, message, ElegantToast.INFO, duration, icon, position, text_mode)

def show_success(parent, message, duration=5000, icon=None, position='top-right', text_mode=ElegantToast.TEXT_WRAP):
    """显示成功提示"""
    return show_toast(parent, message, ElegantToast.SUCCESS, duration, icon, position, text_mode)

def show_warning(parent, message, duration=5000, icon=None, position='top-right', text_mode=ElegantToast.TEXT_WRAP):
    """显示警告提示"""
    return show_toast(parent, message, ElegantToast.WARNING, duration, icon, position, text_mode)

def show_error(parent, message, duration=5000, icon=None, position='top-right', text_mode=ElegantToast.TEXT_WRAP):
    """显示错误提示"""
...
if __name__ == "__main__":
    class MainWindow(QMainWindow):
        ...
        def __init__(self):
            ...
            self.info_button.clicked.connect(lambda: self.show_notification(
                "这是一条信息通知", ElegantToast.INFO))
            button_layout.addWidget(self.info_button)
            
            # 成功通知按钮
            self.success_button = QPushButton("显示成功通知")
            self.success_button.clicked.connect(lambda: self.show_notification(
                "操作已成功完成！", ElegantToast.SUCCESS))
            button_layout.addWidget(self.success_button)
            
            # 警告通知按钮
            self.warning_button = QPushButton("显示警告通知")
            self.warning_button.clicked.connect(lambda: self.show_notification(
                "请注意，这是一个警告！", ElegantToast.WARNING))
            button_layout.addWidget(self.warning_button)
            
            # 错误通知按钮
            self.error_button = QPushButton("显示错误通知")
            self.error_button.clicked.connect(lambda: self.show_notification(
                "发生错误，操作失败！", ElegantToast.ERROR))
        ...
        def setup_global_preload_handler(self):
            ...
            # 连接信号到UI更新
            _global_preload_handler.progress_updated.connect(self.update_preload_progress)
        ...
        def trigger_preload(self):
            ...
            if not _preloading_in_progress:
                ...
                if _global_preload_handler:
                    worker.signals.progress_updated.connect(_global_preload_handler.handle_progress)
        ...
        def show_notification(self, message, toast_type=ElegantToast.INFO, 
                            duration=5000, text_mode=ElegantToast.TEXT_WRAP):
            """显示指定类型的通知"""
            self.toast_manager.show_toast(self, message, toast_type, duration, None, 'top-right', text_mode)
