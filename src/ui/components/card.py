...
class CardWidget(QWidget):
    ...
    def __init__(self, parent=None, primary_color=None, hover_color=None, 
                 text_color=None, selected_color=None, border_radius=12, 
                 title=None, min_width=None, min_height=None,
                 shadow_color=None):
        """
        初始化卡片组件
        ...
        # 默认参数设置 - 使用淡蓝色主题
        self._primary_color = self._parse_color(primary_color) if primary_color else QColor(248, 253, 255)  # 更浅的淡蓝色背景
        ...
        self._text_color = self._parse_color(text_color) if text_color else QColor(50, 50, 50)  # 深灰色文本
        self._border_color = self._primary_color.darker(110)
        self._shadow_color = self._parse_color(shadow_color) if shadow_color else QColor(0, 0, 0, 30)
    ...
    def paintEvent(self, event):
        ...
        if self._selected:
            ...
            # 根据不透明度动画混合主色和悬停色
            r = int(self._primary_color.red() * (1 - self._opacity) + self._hover_color.red() * self._opacity)
            g = int(self._primary_color.green() * (1 - self._opacity) + self._hover_color.green() * self._opacity)
            b = int(self._primary_color.blue() * (1 - self._opacity) + self._hover_color.blue() * self._opacity)
        ...
        if not self._pressed:
            ...
            highlight_rect = button_rect.adjusted(1, 1, -1, button_rect.height() // 4 - 1)
