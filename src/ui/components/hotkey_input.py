...
class VirtualKeyboardDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        for text, key in numpad_top_keys:
            ...
            if text == "NumLk":
                btn.setCheckable(True)
            btn.clicked.connect(
                lambda checked, t=text: self.on_key_selected(
                    f"Num+{t}" if t not in ["NumLk"] else t
                )
            )
            self.key_states[key] = btn
        ...
        numpad_keys = [
            ("7", Qt.Key.Key_7 + 1000, 0, 0),
            ("8", Qt.Key.Key_8 + 1000, 0, 1),
            ("9", Qt.Key.Key_9 + 1000, 0, 2),
            ("4", Qt.Key.Key_4 + 1000, 1, 0),
            ("5", Qt.Key.Key_5 + 1000, 1, 1),
            ("6", Qt.Key.Key_6 + 1000, 1, 2),
            ("1", Qt.Key.Key_1 + 1000, 2, 0),
            ("2", Qt.Key.Key_2 + 1000, 2, 1),
            ("3", Qt.Key.Key_3 + 1000, 2, 2),
            ("0", Qt.Key.Key_0 + 1000, 3, 0, 1, 2),
            (".", Qt.Key.Key_Period + 1000, 3, 2),
        ]
        ...
        for key_data in numpad_keys:
            ...
            btn.clicked.connect(
                lambda checked, t=text: self.on_key_selected(f"Num+{t}")
            )
    ...
    def on_win_clicked(self):
        ...
        for btn in self.findChildren(QPushButton):
            if btn != sender and (
                btn.text() == "Win" or btn.text() == "⌘" or btn.text() == "Super"
            ):
                btn.setChecked(sender.isChecked())
...
class HotkeyInput(QWidget):
    ...
    def __init__(
        ...
        # 样式属性
        self._primary_color = (
            QColor(*primary_color) if primary_color else QColor(13, 110, 253)
        )  # 默认蓝色
        self._focus_color = (
            QColor(*focus_color) if focus_color else QColor(13, 110, 253).lighter(115)
        )
    ...
    def paintEvent(self, event):
        ...
        if self._ripple_radius > 0 and self._ripple_opacity > 0:
            ...
            # 绘制圆形波纹
            painter.drawEllipse(
                self._ripple_position, self._ripple_radius, self._ripple_radius
            )
    ...
    def _setup_ui(self):
        ...
        self._label.setStyleSheet(
            f"color: {self._placeholder_color.name()}; background-color: transparent;"
        )
    ...
    def _show_virtual_keyboard(self):
        ...
        # 启动动画
        self._keyboard_animation_group.start(
    ...
    def set_hotkey(self, hotkey):
        ...
        if hotkey:
            ...
            self._label.setStyleSheet(
                f"color: {self._text_color.name()}; background: transparent;"
            )
        else:
            self._label.setText(self._placeholder)
            self._label.setStyleSheet(
                f"color: {self._placeholder_color.name()}; background: transparent;"
            )
    ...
    def eventFilter(self, obj, event):
        ...
        # 在捕获各类键盘事件时，不让它们继续传播
        if self._is_focused and event.type() in [
            QEvent.Type.KeyPress,
            QEvent.Type.KeyRelease,
            QEvent.Type.ShortcutOverride,
        ]:
...
class HotkeyDemo(QWidget):
    ...
    def init_ui(self):
        ...
        hotkey2, result2 = self.create_hotkey_section(
            "按下组合键...", primary_color=[41, 128, 185], focus_color=[52, 152, 219]
        )
        ...
        hotkey3, result3 = self.create_hotkey_section(
            "设置媒体播放快捷键", primary_color=[46, 204, 113], focus_color=[39, 174, 96]
        )
        ...
        hotkey4, result4 = self.create_hotkey_section(
            "录制新快捷键", primary_color=[231, 76, 60], focus_color=[192, 57, 43]
        )
    ...
    def create_hotkey_section(
        self, placeholder, primary_color=None, focus_color=None, bg_color=None
    ):
        """创建一个快捷键输入区域"""
        # 创建水平布局
        layout = QHBoxLayout()
