...
class AnimatedProgressBar(QProgressBar):
        ...
        if value <= 50:
            ...
            current_color = QColor(
                int(
                    ...
                    + (self._mid_color.red() - self._base_color.red()) * ratio
                ),
                int(
                    self._base_color.green()
                    + (self._mid_color.green() - self._base_color.green()) * ratio
                ),
                int(
                    self._base_color.blue()
                    + (self._mid_color.blue() - self._base_color.blue()) * ratio
                ),
            )
        else:
            # 从黄色到红色的渐变 (50-100%)
            ...
            current_color = QColor(
                int(
                    self._mid_color.red()
                    + (self._high_color.red() - self._mid_color.red()) * ratio
                ),
                int(
                    self._mid_color.green()
                    + (self._high_color.green() - self._mid_color.green()) * ratio
                ),
                int(
                    self._mid_color.blue()
                    + (self._high_color.blue() - self._mid_color.blue()) * ratio
                ),
            )
    ...
    def set_color_theme(self, base_color, mid_color, high_color):
        """设置进度条的颜色主题"""
        self._base_color = (
            QColor(base_color) if isinstance(base_color, str) else QColor(*base_color)
        )
        self._mid_color = (
            QColor(mid_color) if isinstance(mid_color, str) else QColor(*mid_color)
        )
        self._high_color = (
            QColor(high_color) if isinstance(high_color, str) else QColor(*high_color)
        )
