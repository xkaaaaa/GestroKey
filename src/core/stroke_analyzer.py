        ...
        for i in range(1, len(coords)):
            ...
            if i < len(coords) - 1:
                ...
                if len(angle_history) >= 4:
                    ...
                    if (
                        angle_diff > self.min_direction_change
                        and len(current_segment) >= self.min_segment_points
                    ):
    ...
    def _analyze_segment_directions(self, coords):
        ...
        for i in range(1, len(coords)):
            ...
            direction_weights[direction] = (
                direction_weights.get(direction, 0) + segment_length
            )
        ...
        direction_percentages = {
            d: (w / total_length) * 100 for d, w in direction_weights.items()
        }
