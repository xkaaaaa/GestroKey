...
- `ConsoleTab`：控制台选项卡类，继承自`QWidget`
  - `initUI()`：初始化控制台选项卡界面
  - `toggle_drawing()`：切换绘制状态，根据当前状态调用开始或停止绘制功能
  - `start_drawing()`：开始绘制功能
  - `stop_drawing()`：停止绘制功能

**特性说明**：
- 简洁的用户界面，顶部对齐布局，与其他界面保持风格一致
- 单一状态切换按钮，根据当前状态动态变更文本和颜色
- 开始绘制时按钮显示为"停止绘制"，颜色变为红色
- 停止绘制时按钮显示为"开始绘制"，颜色变为蓝色
- **系统监测功能**，提供实时CPU、内存使用率、运行时间和进程资源监测
- 监测信息以精美的卡片形式显示，包含直观的进度条和数值显示
- 自动周期性更新，默认每1.5秒更新一次监测数据
- 可查看详细的内存使用信息（通过鼠标悬停提示）
...
GestroKey中所有界面组件都实现了自适应布局设计，使应用程序能够根据窗口大小自动调整UI元素的布局。这确保了用户在不同分辨率和窗口尺寸下都能获得最佳的使用体验。

**主要特性：**
- **自动调整元素大小**：所有界面元素都会根据可用空间自动调整大小，而不是使用固定尺寸。
- **滚动区域支持**：在空间有限时，界面会自动提供滚动功能，确保所有内容都可访问。
- **合理的空间分配**：使用`QSizePolicy`和`QSpacerItem`确保界面元素在空间调整时保持美观和可用。
- **分割器控件**：手势管理界面使用`QSplitter`控件，允许用户自定义左右区域的宽度比例。
- **最小尺寸限制**：为重要元素设置最小尺寸而不是固定尺寸，确保它们在界面缩小时仍然可用。
- **高DPI支持**：自动适应不同显示器的缩放设置，确保在高分辨率屏幕上显示清晰。

**响应式设计的实现：**
- 每个界面组件都实现了`resizeEvent`方法，监控尺寸变化并做出相应调整。
- 使用`QScrollArea`确保内容在有限空间中完全可见。
- 为组件设置合适的`SizePolicy`以控制其在布局中的行为。
- 使用比例系数而不是固定像素值来分配空间。
- 设置合理的最小窗口尺寸，确保内容不会过度压缩。

通过这些技术，GestroKey在桌面应用软件中实现了类似于响应式网页的自适应体验，无论用户如何调整窗口大小，都能保持良好的布局和可用性。 

### 3.6 core/system_monitor.py

**功能说明**：系统资源监测模块，提供CPU、内存使用情况和程序运行时间等信息。

**主要类和方法**：
- `SystemMonitor`：系统监测器类，继承自`QObject`
  - `__init__(update_interval=1000)`：初始化系统监测器，可指定更新间隔（毫秒）
  - `start()`：开始监测系统信息
  - `stop()`：停止监测系统信息
  - `is_running()`：返回监测器是否正在运行
  - `get_data()`：获取当前监测数据
  - `set_update_interval(interval)`：设置更新间隔
  - `get_update_interval()`：获取当前更新间隔
  - `reset_start_time()`：重置运行时间计时器
...
**特性说明**：
- 自动周期性更新，支持自定义更新间隔
- 基于QTimer的事件驱动设计，不阻塞主线程
- 提供信号机制，方便UI组件实时更新监测数据
- 详细记录各种系统资源使用情况：
  - CPU总体使用率
  - 内存使用率、已用内存和总内存
  - 应用程序运行时间（精确到秒）
...
控制台页面集成了系统监测功能，可实时显示关键系统资源使用情况。本功能通过`core/system_monitor.py`模块实现，并在控制台界面中以美观卡片形式展示。

**系统监测卡片特性：**
- 精美的卡片式设计，使用`CardWidget`实现，每张卡片使用不同主题色
- 实时显示CPU使用率、内存使用率、运行时间和进程资源占用
- 简洁明了的数据展示方式，使用大字体显示关键数值
- 悬停提示显示详细信息（如内存使用的具体数值）

**动画进度条特性：**
- **平滑渐变动画**：数值变化时使用动画效果平滑过渡，而非直接跳变，提供更好的视觉体验
- **智能渐变色系统**：根据使用率自动调整颜色，从绿色(低负载)、黄色(中等负载)到红色(高负载)
- **自定义颜色主题**：支持为不同类型的数据设置不同的颜色主题，如CPU使用率使用绿-黄-红主题，而内存使用率使用蓝色系主题
- **高性能动画实现**：使用`QPropertyAnimation`实现高效流畅的动画效果，过渡时间和缓动曲线经过优化
- **圆角设计**：进度条采用圆角设计，视觉效果现代简约
...
# 设置进度条值（带动画效果）
progress_bar.set_animated_value(75)  # 将以动画方式从当前值过渡到75%

# 自定义颜色主题（可选）
progress_bar.set_color_theme(
    [52, 152, 219],  # 低使用率颜色（如蓝色）
    [41, 128, 185],  # 中等使用率颜色
    [31, 97, 141]    # 高使用率颜色
)
```
...
**数据更新机制：**
- 基于`QTimer`的定时更新，默认每1.5秒刷新一次数据
- 使用信号-槽机制通知UI更新，保持界面响应
- 数据获取使用`psutil`库，确保跨平台兼容性
